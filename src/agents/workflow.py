...
class AgentWorkflow:
    ...
    async def run_analysis(
        self, ticker: str, chunks: List[FilingChunk], max_retries: int = 1
    ) -> AlphaSignal:
        """
        ...
        # Pause between sequential LLM calls to avoid hitting the
        # per-minute input-token rate limit (configurable via .env).
        delay = settings.INTER_LLM_DELAY_SECONDS
        if delay > 0:
            log.info("Pausing before Critic call", delay_seconds=delay)
