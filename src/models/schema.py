...
class AlphaSignal(BaseModel):
    ...
    signal_score: float = Field(
        ...,
        description="0.0 to 1.0 score indicating alpha potential (risk/sentiment drift).",
    )
    confidence: float = Field(
        ..., description="0.0 to 1.0 confidence in the assessment."
    )
    ...
    key_quotes: List[str] = Field(
        default_factory=list, description="Verbatim quotes supporting the signal."
    )
    critic_notes: Optional[str] = Field(
        None,
        description="Notes from the critic agent regarding potential hallucinations or gaps.",
    )
